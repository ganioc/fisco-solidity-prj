...
contract HecoRuffToken is ParameterizedToken {
    address private _lock_ruff_addr; // account for reserve, receive heco token

    event ReserveModification(
        address  previousReserve,
        address  newReserve
    );
    event DepositEvent(address reserve, uint256 amount, string ruffAddr);
    event WithdrawEvent(address reserve, uint256 amount, address to, string txHash);
    ...
    function setLockAddr(address addr) public onlyOwner returns (bool) {
        ...
        emit ReserveModification(_lock_ruff_addr, addr);
        _lock_ruff_addr = addr;

        return true;
    }

    function getLockAddr() public view returns (address) {
        return _lock_ruff_addr;
    }
    ...
    function withdrawTo(
        ...
        transfer(_to, _value);
        emit WithdrawEvent(_lock_ruff_addr, _value, _to, txHash);
        return true;
    }
   ...
   constructor(
        ...
        _lock_ruff_addr = address(0);
    }
}

contract RuffToken is HecoRuffToken {
    constructor() public HecoRuffToken("RUFF", "RUFF", 18, 2000000000) {}
}